"""
This is the GPU implementation of regular CNN model using pytorch on Manufacturing. It is the baseline model used to compare with CPAC-CNN. The codes are similar to train_mnist.py except for the model initilization.
python train_manu_cnn.py [number of filters] [filter_size] [rank] [epochs] [device]; python train_manu_cnn.py 8 3 6 10 cuda:0
"""
...
##result directory       
par_root = "./result"
file_root = par_root + '/' + 'manu_2layer_cnn_'+str(num_filters) + '_' + str(rank) + '_' + str(n_epochs) + '_' + time.strftime("%Y%m%d-%H%M%S")
os.mkdir(file_root)  
